...
public class CollectorProxy implements Collector {
    ...
    @Override
    public void collectEumServerSpans(List<EumServerSpan> serverSpans) throws Exception {
        if (instance == null) {
            if (latch.await(2, MINUTES)) {
                checkNotNull(instance).collectEumServerSpans(serverSpans);
            }
        } else {
            instance.collectEumServerSpans(serverSpans);
        }
    }

    @Override
    public void collectEumClientSpans(List<EumClientSpan> clientSpans) throws Exception {
        if (instance == null) {
            if (latch.await(2, MINUTES)) {
                checkNotNull(instance).collectEumClientSpans(clientSpans);
            }
        } else {
            instance.collectEumClientSpans(clientSpans);
        }
    }
